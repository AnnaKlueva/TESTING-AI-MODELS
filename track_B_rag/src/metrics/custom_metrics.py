...
# ────────────────────── DeepEval BaseMetric (без LLM) ────────────────────


class _ProxyMetric(BaseMetric):
    def __init__(self, threshold: float = 0.6):
        self.threshold = threshold
        self.async_mode = False
        self.include_reason = True
        self.evaluation_model = "embedding-proxy (no LLM)"
...
class FaithfulnessProxyMetric(_ProxyMetric):
    """Проксі Faithfulness: частка тверджень, семантично підкріплених контекстом."""

    def __init__(self, embed_fn, threshold: float = 0.6, support: float = 0.55):
        super().__init__(threshold)
        self.embed_fn = embed_fn
        self.support = support

    def measure(self, tc):
        self.score = faithfulness_semantic(
            tc.actual_output,
            tc.retrieval_context or [],
            self.embed_fn,
            threshold=self.support,
        )
        self.success = self.score >= self.threshold
        self.reason = (
            f"{self.score:.2f} тверджень підкріплено контекстом (проксі, без LLM)"
        )
        return self.score

    @property
    def __name__(self):
        return "FaithfulnessProxy"


class AnswerRelevancyProxyMetric(_ProxyMetric):
    """Проксі Answer Relevancy: косинус питання↔відповідь."""

    def __init__(self, embed_fn, threshold: float = 0.6):
        super().__init__(threshold)
        self.embed_fn = embed_fn

    def measure(self, tc):
        self.score = answer_relevancy_semantic(
            tc.input, tc.actual_output, self.embed_fn
        )
        self.success = self.score >= self.threshold
        self.reason = f"косинус(питання, відповідь)={self.score:.2f} (проксі, без LLM)"
        return self.score

    @property
    def __name__(self):
        return "AnswerRelevancyProxy"
